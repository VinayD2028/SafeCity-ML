"""
Baseline Artificial Neural Network (ANN) — SafeCity-ML
========================================================
...
Description:
    This module implements a baseline feedforward Artificial Neural Network
    (ANN) using TensorFlow/Keras to predict the type of crime (Crime_Type)
    from spatiotemporal and contextual features.

    Architecture:
        Input  → Dense(64, ReLU) → Dense(num_classes, Softmax)
        Optimizer : Adam
    ...
    See 'Hyper Tuned ANN 10 epochs .py' for the deeper, tuned model
    that achieves 92.01% test accuracy with 5 hidden layers and 10 epochs.

Pipeline:
    Load Data → Scale Features → Split (70/20/10) →
    One-Hot Encode Labels → Build ANN → Train → Evaluate → Visualize
"""
...
# This is critical for gradient-based optimizers like Adam to converge quickly.
...
# ---------------------------------------------------------------------------
# Keras' categorical_crossentropy loss requires one-hot encoded labels.
# num_classes = 36 (all unique crime types in the Chicago dataset)
...
# ---------------------------------------------------------------------------
# 6. Build Baseline ANN
# ---------------------------------------------------------------------------
# A single hidden layer network — simple architecture to establish a
# performance baseline before deeper tuning is applied.
model = Sequential([
    Dense(64, activation='relu', input_dim=X_train.shape[1]),  # Hidden layer
    Dense(num_classes, activation='softmax'),                   # Output layer
])

# Compile with Adam optimizer and multi-class cross-entropy loss
model.compile(
    optimizer='adam',
...
print(f'\n[Baseline ANN] Test Loss    : {loss:.4f}')
print(f'[Baseline ANN] Test Accuracy: {accuracy * 100:.2f}%')

# ---------------------------------------------------------------------------
# 9. Predictions & Classification Metrics
# ---------------------------------------------------------------------------
# Predict class probabilities then convert to hard labels via argmax
y_pred_probabilities = model.predict(X_test)
y_pred               = np.argmax(y_pred_probabilities, axis=-1)

# Weighted metrics account for class imbalance across 36 crime types
precision = precision_score(y_test, y_pred, average='weighted')
recall    = recall_score(y_test,    y_pred, average='weighted')
f1        = f1_score(y_test,        y_pred, average='weighted')

print(f'[Baseline ANN] Test Accuracy: {accuracy * 100:.2f}%')
print(f'[Baseline ANN] Precision    : {precision:.2f}')
print(f'[Baseline ANN] Recall       : {recall:.2f}')
print(f'[Baseline ANN] F1-Score     : {f1:.2f}')
...
# ---------------------------------------------------------------------------
# 10. Training vs. Validation Accuracy Curve
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Maps integer-encoded labels back to human-readable crime type names
crime_type_mapping = {
    0 : 'ARSON',                           1 : 'ASSAULT',
...
for i in range(n_classes):
    if i != 10:    # Exclude 'DOMESTIC VIOLENCE' (too few test examples)
        fpr[i], tpr[i], _ = roc_curve(y_test_onehot[:, i], y_test_pred[:, i])
        roc_auc[i]         = auc(fpr[i], tpr[i])

# Micro-average ROC curve aggregates across all classes
...
plt.title('[Baseline ANN] Receiver Operating Characteristic (ROC) Curve')
...
plt.tight_layout()
plt.show()
plt.savefig('baseline_ann_confusion_matrix.png')
